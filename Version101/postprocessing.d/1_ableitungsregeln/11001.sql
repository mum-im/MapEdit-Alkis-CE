...
SELECT
	...
	ST_CurveToLine(st_multi(wkb_geometry)) AS polygon,
...
SELECT
	...
	'Flurstücke' AS thema,
	'ax_flurstueck' AS layer,
	ST_CurveToLine(st_multi( (SELECT st_collect(geom) FROM st_dump( st_intersection(a.wkb_geometry,b.wkb_geometry) ) WHERE geometrytype(geom)='LINESTRING') )) AS line,
...
SELECT
	...
	'ax_flurstueck_nummer' AS layer,
	CASE
	WHEN horizontaleausrichtung='rechtsbündig' THEN ST_CurveToLine(st_multi(st_rotate(st_makeline(st_translate(point, -(2*len), 0.0), st_translate(point, 0.0, 0.0)),drehwinkel,st_x(point),st_y(point))))
	WHEN horizontaleausrichtung='linksbündig' THEN ST_CurveToLine(st_multi(st_rotate(st_makeline(st_translate(point, 0.0, 0.0), st_translate(point, 2*len, 0.0)),drehwinkel,st_x(point),st_y(point))))
	ELSE ST_CurveToLine(st_multi(st_rotate(st_makeline(st_translate(point, -len, 0.0), st_translate(point, len, 0.0)),drehwinkel,st_x(point),st_y(point))))
	END AS line,
	signaturnummer,
	modell
FROM (
...
SELECT
	...
	ST_CurveToLine(st_multi(l.wkb_geometry)) AS line,
