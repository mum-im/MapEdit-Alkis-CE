...
INSERT INTO po_polygons(gml_id,gml_ids,thema,layer,signaturnummer,sn_randlinie,modell,polygon)
	SELECT
		...
		ST_CurveToLine(st_multi(st_simplify(st_union(st_buffer(wkb_geometry,:flur_buffer)), :flur_simplify))) AS polygon
	FROM ax_flurstueck
...
INSERT INTO po_polygons(gml_id,gml_ids,thema,layer,signaturnummer,sn_randlinie,modell,polygon)
	SELECT
		...
		ARRAY['norGIS'] AS modell,
		ST_CurveToLine(st_multi(st_simplify(st_union(st_buffer(polygon, 0.15)), :gemarkung_simplify))) AS polygon
	FROM po_polygons
	JOIN pg_temp.pp_gemarkungen p ON layer LIKE 'ax_flurstueck_flur_'||gemeindezugehoerigkeit_land||gemarkungsnummer||'%' ESCAPE '?'
...
INSERT INTO po_polygons(gml_id,gml_ids,thema,layer,signaturnummer,sn_randlinie,modell,polygon)
	SELECT
		...
		ARRAY['norGIS'] AS modell,
		ST_CurveToLine(st_multi(st_simplify(st_union(st_buffer(polygon,0.20)), :gemeinde_simplify))) AS polygon
	FROM po_polygons
...
INSERT INTO po_polygons(gml_id,gml_ids,thema,layer,signaturnummer,sn_randlinie,modell,polygon)
	SELECT
		...
		ARRAY['norGIS'] AS modell,
		ST_CurveToLine(st_multi(st_simplify(st_union(st_buffer(polygon,0.30)), :kreis_simplify))) AS polygon
	FROM po_polygons
